...
def get_workspace(snapshot: NiriSnapshot, workspace_id: WorkspaceId) -> object | None:
    """Get workspace by id."""
    return snapshot.workspaces.get(workspace_id)


def get_focused_workspace(snapshot: NiriSnapshot) -> object | None:
...
def get_workspaces_on_output(snapshot: NiriSnapshot, output_name: OutputName) -> tuple[object, ...]:
...
def get_active_workspace_on_output(snapshot: NiriSnapshot, output_name: OutputName) -> object | None:
...
def list_workspaces(snapshot: NiriSnapshot) -> list[object]:
    """List all workspaces."""
