...
def get_output(snapshot: NiriSnapshot, name: OutputName) -> object | None:
    """Get output by name."""
    return snapshot.outputs.get(name)


def list_outputs(snapshot: NiriSnapshot) -> list[object]:
    """List all outputs."""
    return list(snapshot.outputs.values())


def get_active_workspace_for_output(snapshot: NiriSnapshot, output_name: OutputName) -> object | None:
