...
def get_window(snapshot: NiriSnapshot, window_id: WindowId) -> WindowState | None:
    """Get window by id."""
    return snapshot.windows.get(window_id)


def get_windows_on_workspace(snapshot: NiriSnapshot, workspace_id: WorkspaceId) -> tuple[WindowState, ...]:
...
def get_floating_windows(snapshot: NiriSnapshot) -> tuple[WindowState, ...]:
...
def list_windows(snapshot: NiriSnapshot) -> list[WindowState]:
    """List all windows."""
